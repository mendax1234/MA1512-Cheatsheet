...
\begin{enumerate}
    ...
    \item (\textbf{Linearity}) Given functions $f(t)$ and $g(t)$, $\mathcal{L}[af(t)+bg(t)]=a\mathcal{L}[f(t)]+b\mathcal{L}[g(t)]$ for all $a,b\in R$.
    \item (\textbf{Differentiation Property}) $\mathcal{L}[t\cdot f(t)]=-\frac{d}{ds}F(s)$. General form $\mathcal{L}[t^n\cdot f(t)]=(-1)^nF^{(n)}(s)$
    \item (\textbf{First Shifting Theorem}) $\mathcal{L}[e^{at}\cdot f(t)]=F(s-a)$
    \item (\textbf{Derivatives}) $\mathcal{y'}=s\mathcal{L}[y]-y(0)$, $\mathcal{L}[y'']=s^2\mathcal{L}[y]-sy(0)-y'(0)$. General form is $\mathcal{L}[f^{(n)}(t)]=s^n\mathcal{L}[f(t)]-s^{n-1}f(0)-\cdots-sf^{(n-2)}(0)-f^{(n-1)}(0)$
\end{enumerate}
