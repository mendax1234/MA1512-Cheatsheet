...
\begin{enumerate}
    ...
    \item (\textbf{Differentiation Property}) $\mathcal{L}[t\cdot f(t)]=-\frac{d}{ds}F(s)$. General form $\mathcal{L}[t^n\cdot f(t)]=(-1)^nF^{(n)}(s)$, where $F(s)=\mathcal{L}[f(t)]$
...
\begin{enumerate}
    \item (\textbf{Use step fucntion to represent the range})
    \begin{itemize}
        \item $1-u(t-1)$ can represent $0<t<1$
